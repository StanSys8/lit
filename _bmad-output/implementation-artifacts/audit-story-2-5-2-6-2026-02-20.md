...
- Fixed: invalid non-array payload for `POST /admin/topics/bulk` now returns `400 VALIDATION_ERROR`.
- Fixed: denied release attempts (`TOPIC_ALREADY_FREE`, `NOT_FOUND`) now write `RELEASE_TOPIC` audit entries with `result: denied`.
- Fixed: backend tests extended for both changes.
- Fixed: frontend flow tests added for story 2.5/2.6 service interactions (`uploadTopicsCsv`, `releaseTopicByAdmin`) and wired into `App`.
