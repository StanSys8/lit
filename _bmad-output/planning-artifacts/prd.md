...
Навчальний заклад (~90 учнів, ~120 тем курсових робіт) розподіляє теми через email. Процес генерує хаотичну переписку через відсутність єдиного джерела правди про доступність тем. Система замінює цей процес єдиним веб-інструментом: учень бачить актуальний список вільних тем і резервує одну. Після резервації тема зникає зі списку для всіх. Система гарантує відсутність дублів навіть при одночасних запитах — учень отримує явну відмову якщо тема зайнята в момент запису. Адмін керує списками та вивантажує CSV зі статусом.
...
- Учень завершує вибір теми за ≤3 хвилини від першого входу
- Жоден учень не може вибрати більше однієї теми (hard constraint на рівні БД)
- При спробі вибрати вже зайняту тему — явне повідомлення про відмову, без silent failures
- Скасування вибору учнем неможливе — тільки адмін може звільнити тему

### Business Success

- 100% учнів проходять процес вибору без email-переписки з адміном
...
3. Максим бачить: "Цю тему щойно вибрав інший учень. Поверніться до списку"
4. Список оновлюється — теми вже немає. Вибирає іншу

### Journey 3: Учень — спроба обходу
...
2. Bulk upload учнів з CSV (ім'я, email) → система генерує паролі
3. Вивантажує CSV з credentials → копіпастить у власний email-клієнт, відправляє вручну
4. Bulk upload тем з CSV (назва, опис)
5. Перевіряє списки → все готово, учні можуть входити

### Journey 5: Адмін — моніторинг і коригування

**Персона:** Той самий викладач, через день після старту.

1. Заходить → дашборд: 47/90 учнів вибрали тему
2. Учень телефонує: "Помилково вибрав не ту тему" → адмін: "Звільнити тему" → тема повертається у список, учень вибирає знову
...
- FR12: Після вибору учень бачить підтвердження з назвою, описом, науковим керівником та кафедрою своєї теми
...
- Адмін CRUD учнів і тем; bulk upload (CSV); звільнення теми
...
- Email-підтвердження учню після вибору теми
